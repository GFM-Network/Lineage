...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if (TTools.HaveAgroMobs) then                                     // if agro mobs are hanging on us, then
        begin                                  
          Engine.Msg('Fighting off mobs', 128);
          Engine.FaceControl(0, true);                                    // turn on combaat in the interface 
          while (TTools.HaveAgroMobs) do delay(555);                      // while there are agro mobs - wait for the bot to kill them
        end;
